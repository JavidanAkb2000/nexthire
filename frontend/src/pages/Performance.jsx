...
import api from '../api'; // API bağlantısı eklendi
...
function MetricIcon({ type }) {
  const common = { width: 24, height: 24, viewBox: '0 0 24 24', fill: 'none', stroke: '#7C4DFF', strokeWidth: 1.8, strokeLinecap: 'round', strokeLinejoin: 'round' };
  if (type === 'file') return <svg {...common}><path d="M14 3H7a2 2 0 00-2 2v14a2 2 0 002 2h10a2 2 0 002-2V8z" /><path d="M14 3v5h5" /><path d="M9 14h6" /><path d="M9 18h6" /></svg>;
  if (type === 'hand') return <svg {...common}><path d="M8 13V8a1 1 0 112 0v5" /><path d="M10 13v-2a1 1 0 112 0v2" /><path d="M12 13v-1a1 1 0 112 0v1" /><path d="M14 13v-1a1 1 0 112 0v3a5 5 0 01-5 5H8a4 4 0 01-4-4v-3a1 1 0 112 0v1" /><circle cx="17" cy="7" r="1.5" /></svg>;
  if (type === 'wallet') return <svg {...common}><rect x="3" y="6" width="18" height="12" rx="3" /><path d="M16 10h5" /><circle cx="16.5" cy="12" r="1" /></svg>;
  return <svg {...common}><rect x="4" y="5" width="16" height="14" rx="4" /><path d="M7 9l5 4 5-4" /></svg>;
}
...
export default function Performance() {
  ...
  // ----------------------------------------------------
  // API STATE'LERİ VE BAĞLANTILARI
  // ----------------------------------------------------
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // Hem genel performans verilerini hem de haftalık çubuk grafik verilerini çekiyoruz
        const [perfRes, weekRes] = await Promise.all([
          api.get('applications/performance/'),
          api.get('applications/dashboard/weekly-activity/')
        ]);
        setPerfData(perfRes.data);
        setWeeklyData(weekRes.data);
      } catch (error) {
        console.error("Performans verileri çekilemedi:", error);
      }
    };
    fetchData();
  }, []);
  ...
  // API Verisi gelene kadar "Loading" göster, React'in çökmesini (Beyaz Ekran) engelle!
  if (!perfData || !weeklyData) {
    return (
      <div className={`min-h-screen font-sans flex items-center justify-center ${shell}`}>
        <p className="text-xl font-semibold opacity-50 animate-pulse">Loading Performance Data...</p>
      </div>
    );
  }

  // ----------------------------------------------------
  // DİNAMİK DEĞER HESAPLAMALARI
  // ----------------------------------------------------

  // 1. METRIC CARDS (Üstteki 4 Kutu)
  const totalAppsDiff = perfData.apps_this_week - perfData.apps_last_week;
  const diffNote = totalAppsDiff > 0 ? `+${totalAppsDiff} this week` : (totalAppsDiff < 0 ? `${totalAppsDiff} this week` : 'No change');

  const metricCards = [
    { title: 'Total Applications', value: perfData.total_applications.toString(), note: diffNote, tone: totalAppsDiff >= 0 ? 'green' : 'red', icon: 'file' },
    { title: 'Offer Rate', value: `${perfData.offer_rate}%`, note: 'Overall rate', tone: perfData.offer_rate > 10 ? 'green' : 'red', icon: 'hand' },
    { title: 'Average Salary', value: `$${Math.round(perfData.average_salary/1000)}k`, note: 'Updated today', tone: 'green', icon: 'wallet' },
    { title: 'Response Rate', value: `${perfData.response_rate}%`, note: 'Excludes No-Response', tone: perfData.response_rate > 20 ? 'green' : 'red', icon: 'mail' },
  ];

  // 2. WEEKLY ACTIVITY (Çubuk Grafik)
  // Backend'den 7 günlük veri geliyorsa al, gelmiyorsa 0 bas.
  const safeWeeklyData = weeklyData?.data?.length > 0 ? weeklyData.data : Array(7).fill({count: 0});
  const weeklyValues = safeWeeklyData.map(item => item.count);

  // 3. STATUS BREAKDOWN (Durum Çubukları ve Listesi)
  const bd = perfData.status_breakdown;
  const breakdown = [
    { label: 'Screening', value: bd.screening, color: '#F59E0B' },
    { label: 'Interview', value: bd.interview, color: '#FACC15' },
    { label: 'Rejected', value: bd.rejected, color: '#FF5757' },
    { label: 'Offers', value: bd.offers, color: '#22C55E' },
    { label: 'No response', value: bd.no_response, color: '#A3A3A3' },
  ];
  const totalBreakdown = perfData.total_applications > 0 ? perfData.total_applications : 1; // 0'a bölünme hatasını engeller

  // 4. OVERVIEW CARDS (En Alttaki Özet Kartları)
  const highestOfferAmount = perfData.highest_offer.amount > 0 ? `$${Math.round(perfData.highest_offer.amount/1000)}k` : 'N/A';
  const highestOfferCompany = perfData.highest_offer.company !== 'N/A' ? perfData.highest_offer.company : 'No offers yet';

  const overviewCards = [
    { title: 'Average Salary', value: `$${Math.round(perfData.average_salary/1000)}k`, note: 'Across all applications' },
    { title: 'Top Role', value: perfData.top_role !== 'N/A' ? perfData.top_role : 'None', note: 'Most applied position' },
    { title: 'Highest Offer', value: highestOfferAmount, note: highestOfferCompany },
    { title: 'Most Active Week', value: `${weeklyData.total_this_week} Applications`, note: 'In the last 7 days' },
  ];
      ...
      <div className="grid min-h-screen grid-cols-[272px_1fr]">
        ...
        <aside className={`flex flex-col border-r ${sidebar}`}>
          ...
          <div className="px-[18px] pt-6">
            <p className={`${softText} uppercase tracking-[0.02em] text-[9px] font-medium`}>Main Menu</p>
          </div>
            ...
            {navItems.map((item) => (
              <Link key={item.label} to={item.to}>
                <div className={`flex min-h-[56px] items-center gap-4 rounded-[16px] border px-6 py-4 ${item.active ? 'relative border-[#504B63] bg-[#2C2345] text-[#8B5CF6]' : isDark ? 'border-transparent text-white/90' : 'border-transparent text-[#171421]'}`}>
                  ...
                  <span className="whitespace-nowrap font-medium text-[16px]">{item.label}</span>
                </div>
              </Link>
            ))}
          </nav>
        ...
        <main className="px-[72px] pb-10 pt-4">
          ...
          <section className="mt-7 grid w-[1090px] grid-cols-[1.08fr_1fr] gap-10">
            ...
            <div className={`h-[430px] rounded-[18px] border ${panel} px-12 py-9 shadow-sm`}>
                ...
                {weeklyValues.map((value, index) => (
                  <div key={index} className="flex flex-col items-center gap-2">
                    ...
                    <div className={`flex h-[182px] w-[48px] items-end rounded-[12px] ${isDark ? 'bg-[#3A2F69]' : 'bg-[#E5E0F2]'} p-[2px]`}>
                      {/* Çubuk uzunluğu matematiksel olarak dinamik yapıldı, Max 180px */}
                      <div
                        className={`w-full rounded-[10px] ${index === 6 ? 'bg-[#7C5CFF]' : 'bg-[#5A46A8]'}`}
                        style={{ height: `${Math.min(180, 40 + value * 15)}px` }}
                      />
                    </div>
              ...
              <p className="mt-5 text-[13px] font-medium text-[#8B5CF6]">{weeklyData.total_this_week} applications this week</p>
              <p className={`mt-2 text-[13px] font-medium ${weeklyData.change_percentage >= 0 ? 'text-[#22E37D]' : 'text-[#FF5757]'}`}>
                 {weeklyData.change_percentage >= 0 ? '↗' : '↘'} {Math.abs(weeklyData.change_percentage)}% compared to last week
              </p>
            </div>

            {/* STATUS BREAKDOWN İLERLEME ÇUBUĞU */}
            <div className={`h-[430px] rounded-[18px] border ${panel} px-12 py-9 shadow-sm`}>
              <h3 className={`text-[22px] font-semibold leading-none ${brightText}`}>Status Breakdown</h3>
              <p className={`${softText} mt-2 text-[13px]`}>Application states overview</p>

              <div className="mt-10">
                <div className="flex h-[28px] overflow-hidden rounded-[4px] bg-gray-100/10">
                  {/* Oranlar % olarak div genişliğine veriliyor */}
                  {breakdown.map((item) => (
                    item.value > 0 && (
                      <div key={item.label} style={{ width: `${(item.value / totalBreakdown) * 100}%`, backgroundColor: item.color }} />
                    )
                  ))}
                </div>
                <p className={`mt-2 text-right text-[13px] ${isDark ? 'text-white/85' : 'text-[#171421]/85'}`}>
                  Total Applications ({perfData.total_applications})
                </p>
              </div>
              ...
              <div className="mt-8 space-y-5">
                {breakdown.map((item) => (
                  <div key={item.label} className="flex items-center gap-4">
                    ...
                    <span className={`text-[16px] ${isDark ? 'text-white/90' : 'text-[#171421]/90'}`}>
