      ...
      <div className="grid min-h-screen grid-cols-[272px_1fr]">
        ...
        <main className="px-[68px] pb-12 pt-8">
          <div className="flex items-center justify-between">
            <Link to="/applications" className={`inline-flex h-[36px] items-center gap-2 rounded-[12px] border px-4 text-[14px] font-medium transition ${secondaryButton}`}>
          ...
          <div className="mt-8">
            <h2 className="text-[28px] font-semibold leading-[34px]">Add New Application</h2>
            <p className={`${softText} mt-1 text-[14px] leading-[20px]`}>Create a new job application entry and track it from the beginning.</p>
          </div>
          ...
          <section className={`mt-10 w-[980px] rounded-[18px] border ${panel} px-10 py-9`}>
            <div className="grid grid-cols-2 gap-6">
              ...
              <div>
                <p className={`${softText} text-[12px] uppercase tracking-[0.08em]`}>Status</p>
              ...
              <div>
                <p className={`${softText} text-[12px] uppercase tracking-[0.08em]`}>Applied Date</p>
                <input
                  type="date"
                  value={form.appliedDate}
                  onChange={(e) => updateField('appliedDate', e.target.value)}
                  className={`mt-2 h-[48px] w-full rounded-[14px] border px-4 text-[15px] outline-none ${isDark ? 'border-[#4A475B] bg-[#2B2A37] text-white' : 'border-[#D4D0DF] bg-[#F6F3FF] text-[#171421]'}`}
                />
              </div>
            </div>
